<?php
$to_do = array(
  "4x or 6x update" => array(
    "desc" => "Update this system to a version between 4.1 - 4.5, or 6.0 - 6.3, using a game card.",
    "link" => "https://plailect.github.io/Guide/cart-update",
    "link-desc" => "Cart Update"
  ),
  "4x to 8x update" => array(
    "desc" => "Update this system to a version between 4.1 - 8.1 using a game card.",
    "link" => "https://plailect.github.io/Guide/cart-update",
    "link-desc" => "Cart Update"
  ),
  "6x update" => array(
    "desc" => "Update this system to a version between 6.0 - 6.3, using a game card.",
    "link" => "https://plailect.github.io/Guide/cart-update",
    "link-desc" => "Cart Update"
  ),
  "90 to 107 update" => array(
    "desc" => "Update this system to a version between 9.0 - 10.7 using a game card. Note that updating to 9.9+ from 9.8 or lower will remove any installed browser!",
    "link" => "https://plailect.github.io/Guide/cart-update",
    "link-desc" => "Cart Update"
  ),
  "fw downgrade 110" => array(
    "desc" => "Downgrade NFIRM from 11.0.0 to 10.4.0. If using DSiWare Downgrade, use <b>dgTool 1.0</b>. This won't change the version shown in System Settings, but will enable downgrading further to 9.2.0.",
    "link" => "https://plailect.github.io/Guide/nfirm-downgrade",
    "link-desc" => "NFIRM Downgrade"
  ),
  "fw downgrade 111" => array(
    "desc" => "Downgrade NFIRM from 11.1.0 to 10.4.0. If using DSiWare Downgrade, use <b>dgTool 1.1</b>. This won't change the version shown in System Settings, but will enable downgrading further to 9.2.0.",
    "link" => "https://plailect.github.io/Guide/nfirm-downgrade",
    "link-desc" => "NFIRM Downgrade"
  ),
  "fw downgrade 112" => array(
    "desc" => "Downgrade NFIRM from 11.2.0 to 10.4.0. If using DSiWare Downgrade, use <b>dgTool 1.2 (currently unavailable!)</b>. This won't change the version shown in System Settings, but will enable downgrading further to 9.2.0.",
    "link" => "https://plailect.github.io/Guide/nfirm-downgrade",
    "link-desc" => "NFIRM Downgrade"
  ),
  "new jpn 81 update" => array(
    "desc" => "Update the system to 9.2.0 using the NTR CFW and Cubic Ninja method.",
    "link" => "https://plailect.github.io/Guide/ntr-and-cubic-ninja",
    "link-desc" => "NTR and Cubic Ninja"
  ),
  "hbl browser" => array(
    "desc" => "Use the Internet Browser (or other exploit) to use the Homebrew Launcher.",
    "link" => "https://plailect.github.io/Guide/homebrew-launcher-(browser)",
    "link-desc" => "Homebrew Launcher (Browser)"
  ),
  "hbl no browser" => array(
    "desc" => "Use one of the exploits below to use the Homebrew Launcher.",
    "link" => "https://plailect.github.io/Guide/homebrew-launcher-(no-browser)",
    "link-desc" => "Homebrew Launcher (No Browser)"
  ),
  "92 downgrade" => array(
    "desc" => "Downgrade to 9.2.0.",
    "link" => "https://plailect.github.io/Guide/9.2.0-downgrade",
    "link-desc" => "9.2.0 Downgrade"
  ),
  "d9 browser" => array(
    "desc" => "Enter Decrypt9, using the Internet Browser.",
    "link" => "https://plailect.github.io/Guide/decrypt9-(browser)",
    "link-desc" => "Decrypt9 (Browser)"
  ),
  "d9 no browser" => array(
    "desc" => "Enter Decrypt9, using the Nintendo DS Profile exploit.",
    "link" => "https://plailect.github.io/Guide/decrypt9-(mset)",
    "link-desc" => "Decrypt9 (MSET)"
  ),
  "ctrtransfer" => array(
    "desc" => "Use the 2.1.0 ctrtransfer method.",
    "link" => "https://plailect.github.io/Guide/2.1.0-ctrtransfer",
    "link-desc" => "2.1.0 ctrtransfer"
  ),
  "install a9lh" => array(
    "desc" => "Install arm9loaderhax.",
    "link" => "https://plailect.github.io/Guide/installing-arm9loaderhax",
    "link-desc" => "Installing arm9loaderhax"
  ),
);
